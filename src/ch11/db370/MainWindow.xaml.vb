...
Class MainWindow
    ...
    Private Sub clickUpdate(sender As Object, e As RoutedEventArgs)
        ...
        If _vm.Item.Id = 0 Then
            ...
            ' 新規登録の場合
            If context.Book.Any(Function(t) t.Title = _vm.Item.Title) Then
        ...
        Else
            ...
            Dim item = context.Book.FirstOrDefault(Function(t) t.Title = _vm.Item.Title)
            If Not item Is Nothing And item.Id <> _vm.Item.Id Then
                tr.Rollback()
                MessageBox.Show("書名が重複しているためロールバックしました")
                Return
            End If
