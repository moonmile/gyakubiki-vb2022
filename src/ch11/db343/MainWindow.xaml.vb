...
Class MainWindow
    ...
    ''' <param name="e"></param>
    Private Sub clickSearchByTitle(sender As Object, e As RoutedEventArgs)
        ...
        Dim items = context.Book.Join(context.Author,
                ...
                .OrderBy(Function(t) t.book.Title) _
    ...
    ''' <param name="sender"></param>
    ''' <param name="e"></param>
    Private Sub clickSearchByAuthor(sender As Object, e As RoutedEventArgs)
        Dim context As New MyContext()
        Dim items = context.Book.Join(context.Author,
                    Function(book) book.AuthorId,
                    Function(author) author.Id,
                    ...
                    Function(publisher) publisher.Id,
                    Function(t, publisher) New With {t.book, t.author, publisher}) _
                .OrderBy(Function(t) t.author.Name) _
                .Select(Function(t) New With {
                    .Id = t.book.Id,
                    .Title = t.book.Title,
                    .AuthorName = t.author.Name,
